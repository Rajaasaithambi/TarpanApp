...
open class FloatInputView: UIView {
  open static var stringTransformer: ((String) -> String?)! = {
    orginal in
    return orginal
  }
  open static var instanceTransformer: ((FloatInputView) -> Void)! = {
    orginal in
  }
  ...
  @IBInspectable open var placeHolderStringKey: String = "" {
    didSet {
      placeHolderLabel.string = FloatInputView.stringTransformer(placeHolderStringKey)
    }
  }
  @IBInspectable open var placeHolderFontKey: String = "BradleyHandITCTT-Bold"
  @IBInspectable open var idlePlaceHolderFontSize: CGFloat = 16
  @IBInspectable open var floatPlaceHolderFontSize: CGFloat = 14
  @IBInspectable open var inputFontName: String = "BradleyHandITCTT-Bold"
  ...
  override open func awakeFromNib() {
    super.awakeFromNib()
    self.addObserver(self, forKeyPath: "textField.text", options: [.new, .old], context: nil)
    build()
  }
  ...
  open func build() {
    ...
    if applyTransform { FloatInputView.instanceTransformer(self) }
  ...
  open func editingDidEnd() {
    ...
    deinit {
        self.removeObserver(self, forKeyPath: "textField.text")
    }
